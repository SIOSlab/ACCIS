...
*===============================================================================
      ...
      REAL FUNCTION DENSTY(DT, DRGC, DF107, DF107A, DAP)

*-------------------------------------------------------------------------------
*                   Atmospheric Density (NRLMSISE-00)
*
*  IN:  DT     - Time (JD/UT)
*       DRGC   - Geocentric position vector (km, WGS84 frame)
*       DF107  - 10.7 cm solar flux (sfu), previous day average
*       DF107A - 10.7 cm solar flux (sfu), 81-day average
*       DAP    - Geomagnetic index, daily average
*
*  OUT: Total mass density (kg/m^3)
*
*-------------------------------------------------------------------------------

*  Declarations
      IMPLICIT DOUBLE PRECISION (D)
      DIMENSION DRGC(3), NSW(25), RHO(9), T(2) 
      LOGICAL SETUP
      SAVE SETUP
      DATA SETUP /.FALSE./
...
*  Day of year & second of day
      NDAY = INT(DMOD(DT-0.5D0, 365.25D0))
      DSEC = 86400D0 * DMOD(DT-0.5D0, 1D0)

*  Geodetic latitude, longitude, and altitude
      CALL GEOD(DRGC, 1D-2, DLAT, DLON, DALT)

*  Local standard time
      DLST = DSEC/3600 + DLON/15

*  Convert double precision parameters to real
      F107  = REAL(DF107)
      F107A = REAL(DF107A)
      AP    = REAL(DAP)
      GLAT  = REAL(DLAT)
...
* Compute density using NRLMSISE-00 Model
      IF (GALT .GT. 1000.0) THEN
          DENSTY = 0
      ELSE
          IF (GALT .GT. 500.0) THEN
              CALL GTD7D(NDAY, SEC, GALT, GLAT, GLON, TLST, F107A, F107,
     2            AP, 48, RHO, T)
          ELSE
              CALL GTD7(NDAY, SEC, GALT, GLAT, GLON, TLST, F107A, F107,
     2            AP, 48, RHO, T)
          ENDIF
          DENSTY = RHO(6)
      ENDIF
